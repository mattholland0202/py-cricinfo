...
@router.get(
    "/{player_id}/{player_name}/career",
    responses={status.HTTP_200_OK: {"description": "The Player career stats"}},
    summary="Get Player career stats",
)
async def player_career(
    player_name: str = Path(description="The Player Name"), player_id: int = Path(description="The Player ID")
) -> Career:
    return await get_player_career_stats(player_name=player_name, player_id=player_id)
