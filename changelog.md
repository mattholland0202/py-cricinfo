...
## [0.0.39]
### Fixed
- Improve comments on player stats code
- Add lenient optional type to handle cases where API responses return empty dicts rather than null, specifically in Commentary response

## [0.0.38]
### Fixed
- Handle Career stats for Players who have only played one format
...
## [0.0.36]
### Fixed
- Use correct types in Career model
- Missing `.gitattributes` file so GitHub ignores test files in language checking

## [0.0.35]
### Added
- Parsing of a Player page to get Career stats

## [0.0.34]
### Added
- Missing fields to Player model

## [0.0.33]
### Added
- Friendly names to `MatchNoteType`
- Tests for ball-by-ball data

### Changed
- Sorted some missing fields and renamed some classes in processing ball-by-ball Commentary

## [0.0.32]
### Fixed
- Correctly parse `playerreplacement` Match Notes, as they have different fields

## [0.0.31]
### Fixed
- Some missing defaults for optional fields

### Changed
- Combined models between `match` and `match_basic`, removing the separate file for the latter
...
### Changed
- Moved some imports and altered function names to improve clarity
...
## [0.0.26]
### Fixed
- Handle `absent hurt` and `retired hurt` in scorecards

## [0.0.25]
### Added
- Some extra attributes to innings
...
## [0.0.24]
### Added
- Missing match note type
- `DeliveryPlayTypes` for ball-by-ball data
